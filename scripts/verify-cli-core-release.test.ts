...
// console#264 (carried over from kontourai/console): verify-cli-core-release.ts
// must (Route A) read package manifests from the invoking process's working
// directory — NOT from its own on-disk location — so publish-npm.yml can
// execute a fixed copy of this script from a separate `cli-main` checkout
// while manifest data stays authoritative from the immutable tag checkout.
// And (Route B) it must tolerate npm CLI output-shape drift (array-wrapped
// single result) without failing closed on shape alone.
...
// this mirrors the real workflow, where `working-directory` cwd has its own
// `npm ci`-installed node_modules.
...
test("root resolves from process.cwd(), not the script's own directory, so a main-checked-out copy still reads the tag's manifest (Route A)", () => {
  ...
  try {
    ...
    const tagCheckout = join(root, "cli");
    mkdirSync(tagCheckout, { recursive: true });
    writeFileSync(join(tagCheckout, "package.json"), JSON.stringify({ name: "@kontourai/cli", dependencies: { "@kontourai/console-core": "0.3.0" } }));
...
test("array-wrapped npm@latest output shape (console#264 root cause) no longer fails the gate (Route B)", () => {
  const root = mkdtempSync(join(tmpdir(), "verify-cli-core-release-"));
  try {
    writeFileSync(join(root, "package.json"), JSON.stringify({ name: "@kontourai/cli", dependencies: { "@kontourai/console-core": "0.3.0" } }));
